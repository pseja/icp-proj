/**
 * @file fsm.cpp
 * @brief Implements the FSM class, which represents a finite state machine.
 *
 * This file contains the implementation of the FSM class, which extends QStateMachine
 * and provides functionality for managing states, transitions, variables, inputs, and outputs.
 * The FSM class also supports querying transitions by their source or target states and allows
 * for dynamic state machine construction.
 *
...
QSet<QString> FSM::getInputs()
{
    return inputs;
}

QSet<QString> FSM::getOutputs()
{
    return outputs;
}

QMap<QString, Variable *> FSM::getVariables()
{
    return variable_map;
}

QMap<QString, State *> FSM::getStates()
{
    return state_map;
}

QMultiMap<QString, Transition *> FSM::getTransitions()
{
    return from_transition_map;
}

QMultiMap<QString, Transition *> FSM::getTransitionsFrom()
{
    return from_transition_map;
}

QMultiMap<QString, Transition *> FSM::getTransitionsTo()
{
    return to_transition_map;
}

QList<Transition *> FSM::getTransitionsFrom(State *state)
{
    return from_transition_map.values(state->getName());
}

QList<Transition *> FSM::getTransitionsTo(State *state)
{
    return to_transition_map.values(state->getName());
}

QList<Transition *> FSM::getTransitionsFrom(QString state_name)
{
    return from_transition_map.values(state_name);
}

QList<Transition *> FSM::getTransitionsTo(QString state_name)
{
    return to_transition_map.values(state_name);
}

void FSM::setName(QString new_name)
{
    name = new_name;
}

void FSM::setComment(QString new_comment)
{
    comment = new_comment;
}
...
{
    state_map[new_state->getName()] = new_state;
}

void FSM::addState(State *new_state, QString new_name)
{
    state_map[new_name] = new_state;
}

void FSM::addInput(QString new_input)
{
    inputs.insert(new_input);
}

void FSM::addOutput(QString new_output)
{
    outputs.insert(new_output);
}
...
{
    variable_map[new_variable->getName()] = new_variable;
}
